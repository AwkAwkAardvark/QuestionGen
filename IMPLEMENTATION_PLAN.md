...
- [x] Make `BOOTSTRAP_ENV=True` a bootstrap-only step that stops before repo refresh or in-kernel execution so fresh Colab runtimes do not mix package installs with same-pass app or batch launch.
...
- [x] `vocab`
  ...
  - current hard-family admission and prompting policy:
    ...
    - [x] for `contextual_vocab_error_1_among_5_collocation_5`, lock a five-target bundle that includes an explicit collocation-eligible corruption subset and fail early as `qtype_incompatibility_error` when no such local collocational anchor exists
